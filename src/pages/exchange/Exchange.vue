<template>
  <div class="container exchange" :class="skin">
    ...
    <div class="symbol">
      ...
      <div class="item">
        <span class="text">{{$t('add.newest')}}</span>
        <span class="num" :class="{buy:currentCoin.change>0,sell:currentCoin.change<0}">{{currentCoin.close | toFixed(baseCoinScale)}}</span>
        <span class="price-cny">${{currentCoin.usdRate | toFixed(2)}}</span>
      </div>
      <div class="item">
        <span class="text">{{$t('add.hour24')}}</span>
        <span class="num" :class="{buy:currentCoin.chg>0,sell:currentCoin.chg<0}">{{currentCoin.rose}}</span>
      </div>
      <div class="item">
        <span class="text">{{$t('add.highest')}}</span>
        <span class="num ">{{currentCoin.high | toFixed(baseCoinScale)}}</span>
      </div>
      <div class="item">
        <span class="text">{{$t('add.lowest')}}</span>
        <span class="num ">{{currentCoin.low | toFixed(baseCoinScale)}}</span>
      </div>
      <div class="item">
        <span class="text">{{$t('add.transaction')}}</span>
    ...
    <div class="main">
      <div class="left plate-wrap">
        ...
        <div class="plate-nowprice">
          ...
          <span class="price-cny"> ≈ {{currentCoin.usdRate | toFixed(2)}} USD</span>
        </div>
            ...
            <div class="mask" v-show="isLogin&&!member.realName">
              <span>{{$t('add.add_20')}}
                <router-link to="/uc/safe">
                  <span style="color:#f0a70a;">{{$t('add.add_19')}}</span>
                </router-link>
              </span>
            </div>
      ...
      <div class="right">
        <div class="coin-menu">
          <div class="sc_filter">
            ...
            <span @click="changeBaseCion('usdt')" :class="{active:basecion==='usdt'}" style="width:25%">USDT</span>
            <span @click="changeBaseCion('btc')" :class="{active:basecion==='btc'}" style="width:25%">BTC</span>
            <span @click="changeBaseCion('eth')" :class="{active:basecion==='eth'}" style="width:25%">ETH</span>
            <!-- <span v-show="isLogin" @click="changeBaseCion('favor')" :class="{active:basecion==='favor'}" style="width:25%">{{$t('add.select')}}</span> -->
            <!-- <span :class="{active:basecion==='favor'}">自选</span> -->
            <!-- <Icon style="line-height:32px;" type="android-star"></Icon> -->
          </div>
          <Table @on-current-change="gohref" highlight-row id="QC" v-show="basecion==='qc'" :columns="coins.columns" :data="coins.QC" :no-data-text="$t('common.nodata')"></Table>
          <Table @on-current-change="gohref" highlight-row id="USDT" v-show="basecion==='usdt'" :columns="coins.columns" :data="coins.USDT" :no-data-text="$t('common.nodata')"></Table>
          <Table @on-current-change="gohref" highlight-row id="BTC" v-show="basecion==='btc'" :columns="coins.columns" :data="coins.BTC" :no-data-text="$t('common.nodata')"></Table>
          <Table @on-current-change="gohref" highlight-row id="ETH" v-show="basecion==='eth'" :columns="coins.columns" :data="coins.ETH" :no-data-text="$t('common.nodata')"></Table>
          <Table @on-current-change="gohref" highlight-row v-show="basecion==='favor'" :no-data-text="$t('common.nodata')" id="collect" :columns="favorColumns" :data="coins.favor"></Table>
        </div>
        <div class="trade-wrap">
          <Table height="353" :columns="trade.columns" :data="trade.rows" :no-data-text="$t('add.notdata')"></Table>
        </div>
      </div>
    </div>
    <div class="order" v-show="isLogin">
      <div class="order-handler">
        ...
        <router-link v-show="selectedOrder==='current'" class="linkmore" to="/uc/entrust/current">{{$t('add.more')}}</router-link>
        <router-link v-show="selectedOrder==='history'" class="linkmore" to="/uc/entrust/history">{{$t('add.more')}}</router-link>
      </div>
      <div class="table">
        <Table v-if="selectedOrder==='current'" :columns="currentOrder.columns" :data="currentOrder.rows" :no-data-text="$t('common.nodata')"></Table>
        <Table v-else :columns="historyOrder.columns" :data="historyOrder.rows" :no-data-text="$t('common.nodata')"></Table>
      </div>
    </div>
...
.exchange .ivu-input[disabled], .exchange fieldset[disabled] .ivu-input {
  background:#0b1520!important;
}
.trade_wrap .ivu-input {
  background:#0b1520!important;
}
.coin-menu .ivu-table-row span{
  color:#9aa5b5;
}
  ...
  .main {
    ...
    .trade_wrap {
      ...
      .trade_panel {
        box-shadow: 0 0 2px #ccc;
        .mask {
          background-color: rgba(40,49,62,.8);
          color: #fff;
        }
      }
      .trade_panel .panel .hd {
...
export default {
    ...
    return {
      ...
      currentOrder: {
        columns: [
          ...
          {
            title: self.$t('add.transactionPair'),
            key: "symbol"
          },
          {
            title: self.$t('add.transactionPair'),
  ...
  methods: {
    ...
    getKline() {
      ...
      let config = {
        ...
        overrides: {
          ...
          "paneProperties.vertGridProperties.color": "rgba(40,49,62,.8)",
          "paneProperties.horzGridProperties.color": "rgba(40,49,62,.8)",
      ...
      if (that.skin === "night") {
        config.toolbar_bg = "#192330";
        config.custom_css_url = "bundles/common.css";
        config.overrides["paneProperties.background"] = "#192330";
    ...
    startWebsock() {
      ...
      var that = this;
      // var socket = new SockJS("http://47.244.45.34:80" + that.api.market.ws);
      var socket = new SockJS("http://jingui360.com" + that.api.market.ws);
      // var socket = new SockJS("http://127.0.0.1:9595" + that.api.market.ws);
      // var socket = new SockJS("http://192.168.1.28:9595" + that.api.market.ws);
      //var socket = new SockJS("http://192.168.0.2:9595" + that.api.market.ws);
